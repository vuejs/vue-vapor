export const template = (str: string): (() => ChildNode[]) => {
  let cached = false
  ...
  return () => {
    if (!cached) {
      ...
      return toChildNodesArray((node = t.content))
    } else {
...
function toChildNodesArray(node: DocumentFragment) {
